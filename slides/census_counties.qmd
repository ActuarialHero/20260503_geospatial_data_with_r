...
census_state %>%
  filter(STATEFP <= 56) %>% # Remove territories
  leaflet() %>%
  addTiles() %>%
  addPolygons(weight = 0.5, opacity = 1.0) %>%
  setView(lng = -103.75, lat = 44.7, zoom = 1) # Center of U.S.
  # Use fitBounds(lng1, lat1, lng2, lat2) instead?
```


::: {.notes}
We can filter out the territories using our STATEFP code. States and DC are <= 56
We'll adjust the map so Alaska doesn't wrap around
:::
